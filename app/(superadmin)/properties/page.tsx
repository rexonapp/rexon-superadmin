...
function ImageGallery({ images, loading }: { images: WarehouseImage[]; loading: boolean }) {
  ...
  if (loading) return (
    <div className="w-full h-52 rounded-xl flex flex-col items-center justify-center gap-3 bg-brand-teal/8 border-2 border-dashed border-brand-teal/25">
          ...
          {images.map((thumb, i) => (
            <button key={thumb.id} onClick={() => setCurrent(i)}
              className={cn('flex-shrink-0 w-12 h-9 rounded-lg overflow-hidden border-2 transition-all',
                i === current ? 'border-brand-teal scale-105 shadow-sm' : 'border-gray-200 opacity-60 hover:opacity-90')}>
              {imgError[i]
...
function RowThumb({ warehouse }: { warehouse: Warehouse }) {
  ...
  if (!primaryImg || error) {
    return (
      <div className="w-10 h-10 rounded-lg bg-brand-teal/8 flex items-center justify-center flex-shrink-0 border border-brand-teal/15">
        <Building2 className="w-5 h-5 text-brand-teal-medium" />
      </div>
    );
  }
  return (
                  ...
                  <TableCell className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-900 group-hover:text-brand-teal-medium transition-colors leading-snug truncate max-w-[175px]">
                          {w.title}
                        </p>
            ...
            <div className="flex items-start justify-between gap-4 mb-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  <span className="text-white/90 text-xs font-bold uppercase tracking-wide bg-white/15 rounded-full px-2.5 py-1">
              ...
              <div className="flex flex-col items-end gap-1.5 py-4 flex-shrink-0">
                ...
                {/* <p className="text-white/70 text-xs font-mono">#{selected?.id?.slice(0, 8)}</p> */}
              </div>
            </div>
            ...
            {activeTab === 'location' && (
              <div className="max-w-lg">
                ...
                {selected?.latitude && selected?.longitude && (
                  <div className="mt-5 bg-brand-teal/8 border border-brand-teal/25 rounded-xl p-4 flex items-center justify-between gap-4">
                    <div>
                      <p className="text-xs font-bold text-brand-teal-medium uppercase tracking-wider mb-1">Coordinates</p>
                    ...
                    <a href={`https://www.google.com/maps?q=${selected.latitude},${selected.longitude}`}
                      ...
                      className="flex items-center gap-1.5 text-sm font-bold text-white bg-brand-teal-deep hover:bg-brand-teal-dark px-4 py-2 rounded-lg transition-colors flex-shrink-0">
